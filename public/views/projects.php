<main class="WrapperProjects">
  <div class="WrapperProjects__container">
    <div class="ProjectsHeader">
      <p>Veja nossos projetos</p>
      <h1>Compre Projetos</h1>
    </div>

    <div class="ProjectsContent">
      <div class="ProjectsContent__filter" id="projects-filter-area">
        <div class="ProjectsContentFilter">
          <form id="projects-filter">
            <div class="ProjectsContentFilter__title">
              <p>Todos os Filtros</p>
            </div>

            <div class="ProjectsContentFilter__inputs">
              <label class="Input">
                <span class="Input__label">Metros Quadrados</span>
                <div class="Input__content">
                  <div class="Input__content_icon">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><!--!Font Awesome Free 6.5.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M32 32C14.3 32 0 46.3 0 64v96c0 17.7 14.3 32 32 32s32-14.3 32-32V96h64c17.7 0 32-14.3 32-32s-14.3-32-32-32H32zM64 352c0-17.7-14.3-32-32-32s-32 14.3-32 32v96c0 17.7 14.3 32 32 32h96c17.7 0 32-14.3 32-32s-14.3-32-32-32H64V352zM320 32c-17.7 0-32 14.3-32 32s14.3 32 32 32h64v64c0 17.7 14.3 32 32 32s32-14.3 32-32V64c0-17.7-14.3-32-32-32H320zM448 352c0-17.7-14.3-32-32-32s-32 14.3-32 32v64H320c-17.7 0-32 14.3-32 32s14.3 32 32 32h96c17.7 0 32-14.3 32-32V352z"/></svg>
                  </div>
                  <div class="Input__content_input">
                    <input type="text" name="square_meters" id="square_meters" placeholder="200m²">
                  </div>
                </div>
              </label>

              <label class="Input">
                <span class="Input__label">Banheiros</span>
                <div class="Input__content">
                  <div class="Input__content_icon">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--!Font Awesome Free 6.5.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M64 131.9C64 112.1 80.1 96 99.9 96c9.5 0 18.6 3.8 25.4 10.5l16.2 16.2c-21 38.9-17.4 87.5 10.9 123L151 247c-9.4 9.4-9.4 24.6 0 33.9s24.6 9.4 33.9 0L345 121c9.4-9.4 9.4-24.6 0-33.9s-24.6-9.4-33.9 0l-1.3 1.3c-35.5-28.3-84.2-31.9-123-10.9L170.5 61.3C151.8 42.5 126.4 32 99.9 32C44.7 32 0 76.7 0 131.9V448c0 17.7 14.3 32 32 32s32-14.3 32-32V131.9zM256 352a32 32 0 1 0 0-64 32 32 0 1 0 0 64zm64 64a32 32 0 1 0 -64 0 32 32 0 1 0 64 0zm0-128a32 32 0 1 0 0-64 32 32 0 1 0 0 64zm64 64a32 32 0 1 0 -64 0 32 32 0 1 0 64 0zm0-128a32 32 0 1 0 0-64 32 32 0 1 0 0 64zm64 64a32 32 0 1 0 -64 0 32 32 0 1 0 64 0zm32-32a32 32 0 1 0 0-64 32 32 0 1 0 0 64z"/></svg>
                  </div>
                  <div class="Input__content_input">
                    <input type="text" name="bathrooms" id="bathrooms" placeholder="2">
                  </div>
                </div>
              </label>

              <label class="Input">
                <span class="Input__label">Quartos</span>
                <div class="Input__content">
                  <div class="Input__content_icon">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512"><!--!Font Awesome Free 6.5.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M32 32c17.7 0 32 14.3 32 32V320H288V160c0-17.7 14.3-32 32-32H544c53 0 96 43 96 96V448c0 17.7-14.3 32-32 32s-32-14.3-32-32V416H352 320 64v32c0 17.7-14.3 32-32 32s-32-14.3-32-32V64C0 46.3 14.3 32 32 32zm144 96a80 80 0 1 1 0 160 80 80 0 1 1 0-160z"/></svg>
                  </div>
                  <div class="Input__content_input">
                    <input type="text" name="bedrooms" id="bedrooms" placeholder="3">
                  </div>
                </div>
              </label>

              <label class="Input">
                <span class="Input__label">Garagens</span>
                <div class="Input__content">
                  <div class="Input__content_icon">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--!Font Awesome Free 6.5.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M135.2 117.4L109.1 192H402.9l-26.1-74.6C372.3 104.6 360.2 96 346.6 96H165.4c-13.6 0-25.7 8.6-30.2 21.4zM39.6 196.8L74.8 96.3C88.3 57.8 124.6 32 165.4 32H346.6c40.8 0 77.1 25.8 90.6 64.3l35.2 100.5c23.2 9.6 39.6 32.5 39.6 59.2V400v48c0 17.7-14.3 32-32 32H448c-17.7 0-32-14.3-32-32V400H96v48c0 17.7-14.3 32-32 32H32c-17.7 0-32-14.3-32-32V400 256c0-26.7 16.4-49.6 39.6-59.2zM128 288a32 32 0 1 0 -64 0 32 32 0 1 0 64 0zm288 32a32 32 0 1 0 0-64 32 32 0 1 0 0 64z"/></svg>
                  </div>
                  <div class="Input__content_input">
                    <input type="text" name="garages" id="garages" placeholder="1">
                  </div>
                </div>
              </label>

              <label class="Input">
                <span class="Input__label">Preço Mínimo</span>
                <div class="Input__content">
                  <div class="Input__content_icon">
                    <p>R$</p>
                  </div>
                  <div class="Input__content_input">
                    <input type="text" name="min" id="min" placeholder="0,00">
                  </div>
                </div>
              </label>

              <label class="Input">
                <span class="Input__label">Preço Máximo</span>
                <div class="Input__content">
                  <div class="Input__content_icon">
                    <p>R$</p>
                  </div>
                  <div class="Input__content_input">
                    <input type="text" name="max" id="max" placeholder="10,00">
                  </div>
                </div>
              </label>
              
              <button type="submit" class="Button">Filtrar</button>
            </div>
          </form>
        </div>

        <div class="ProjectsButtonFilter">
          <button class="Button" id="button-filter-projects">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--!Font Awesome Free 6.5.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M3.9 54.9C10.5 40.9 24.5 32 40 32H472c15.5 0 29.5 8.9 36.1 22.9s4.6 30.5-5.2 42.5L320 320.9V448c0 12.1-6.8 23.2-17.7 28.6s-23.8 4.3-33.5-3l-64-48c-8.1-6-12.8-15.5-12.8-25.6V320.9L9 97.3C-.7 85.4-2.8 68.8 3.9 54.9z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><!--!Font Awesome Free 6.5.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z"/></svg>
          </button>
        </div>
      </div>

      <div class="ProjectsContent__products">
        <div class="ProjectsContent__products_search">
          <label class="Input">
            <span class="Input__label">Busque projetos</span>
            <div class="Input__content">
              <div class="Input__content_input Input__content_input--padding-x-max">
                <input type="text" name="search" id="search" placeholder="Ex: General Construction">
              </div>
              <button class="Input__content_icon Input__content_icon--border-right Input__content_icon--button" id="search-button">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--!Font Awesome Free 6.5.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M416 208c0 45.9-14.9 88.3-40 122.7L502.6 457.4c12.5 12.5 12.5 32.8 0 45.3s-32.8 12.5-45.3 0L330.7 376c-34.4 25.2-76.8 40-122.7 40C93.1 416 0 322.9 0 208S93.1 0 208 0S416 93.1 416 208zM208 352a144 144 0 1 0 0-288 144 144 0 1 0 0 288z"/></svg>
              </button>
            </div>
          </label>
        </div>

        <div class="ProjectsContent__products_list"></div>

        <div class="ProjectsContent__products_pagination">
          <p>Quantidade de projetos <span id="pages-qtd"></span></p>

          <div>
            <ul id="list-pages">
              <li><p>01</p></li>
              <li><p>02</p></li>
              <li><p>03</p></li>
            </ul>
          </div>
        </div>
      </div>
    </div>
  </div>
</main>