<main class="AdmUsersForm">
  ...
  <form class="row g-3">
    <div class="col-md-6">
      ...
      <input type="text" class="form-control" id="name" value="<?= (isset($user) && !empty($user['name'])) ? $user['name'] : '' ?>">
    </div>
    <div class="col-md-6">
      <label for="email" class="form-label">E-mail</label>
      <input type="email" class="form-control" id="email" value="<?= (isset($user) && !empty($user['email'])) ? $user['email'] : '' ?>">
    </div>
    <div class="col-md-6">
    ...
    <div class="col-md-6">
      <label for="email" class="form-label">Tipo de Usuário</label>
      <select class="form-select" aria-label="Default select example">
        <option selected>Seu tipo de usuário</option>
        <option value="1">Administrador</option>
        <option value="2">Cliente</option>
      </select>
    </div>
    <div class="col-md-12 d-flex justify-content-end gap-3">
      <button type="button" class="btn btn-outline-success"><?= (isset($user)) ? 'Editar o Usuário' : 'Adicionar Usuário +' ?></button>
      <?= (isset($user)) ? '<button type="button" class="btn btn-outline-danger">Deletar Usuário</button>' : '' ?>
    </div>
  </form>
</main>