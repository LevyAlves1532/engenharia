<main class="AdmUsersForm">
  <div class="AdmBreadcrumbs mb-3">
    ...
    <a href="<?= BASE ?>admin/users">Usuários</a>
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><!--!Font Awesome Free 6.5.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M310.6 233.4c12.5 12.5 12.5 32.8 0 45.3l-192 192c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3L242.7 256 73.4 86.6c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0l192 192z"/></svg>
    <p>Formulário</p>
  </div>
  ...
  <div class="AdmHeaderPage mb-5">
    <div class="AdmHeaderPage__title">
      <h1><?= (isset($user)) ? 'Editar' : 'Adicionar' ?></h1>
    </div>
  </div>

  <form class="row g-3" id="users-form">
    <?= (isset($user)) ? '<input type="hidden" name="iu" value="' . base64_encode($user['id']) . '" />' : '' ?>
    <div class="col-md-6">
      <label for="name" class="form-label">Nome:</label>
      <input type="text" class="form-control" name="name" id="name" value="<?= (isset($user) && !empty($user['name'])) ? $user['name'] : '' ?>">
      <div class="error"></div>
    </div>
    <div class="col-md-6">
      <label for="email" class="form-label">E-mail</label>
      <input type="email" class="form-control" name="email" id="email" value="<?= (isset($user) && !empty($user['email'])) ? $user['email'] : '' ?>">
      <div class="error"></div>
    </div>
    <div class="col-md-6">
    ...
    <div class="col-md-12 d-flex justify-content-end gap-3">
      <button type="submit" class="btn btn-outline-success"><?= (isset($user)) ? 'Editar o Usuário' : 'Adicionar Usuário +' ?></button>
      <?= (isset($user)) ? '<a href="' . BASE . 'admin/users/delete/' . $id . '" type="submit" class="btn btn-outline-danger">Deletar Usuário</a>' : '' ?>
    </div>
  </form>
</main>